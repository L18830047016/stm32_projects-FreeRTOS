...
void Start_Task(void *pvParameters){ 
  ...
  taskENTER_CRITICAL();
  //创建信号量（计数）
  semaphore=xSemaphoreCreateCounting(5,5);//刚创建完时，信号量中是满的
  if(semaphore==NULL){
    printf("%s\n","semphr create fail");
  }else{
...
void Rec_Task(void *pvParameters){
 uint8_t keyNum = 0;
  ...
  while(1){
   ...
   if(keyNum==2){
    printf("%s\n","key2 click,receive semphr\n");
    res=xSemaphoreTake(semaphore,0);
      if(res==pdTRUE){
        printf("key2 receive semphr success,rest semphr num is %ld\n",uxQueueMessagesWaiting(semaphore));
      }else{
        printf("key2 receive semphr fail\n");
      }
   }
  ...
  while(1){
   ...
   if(keyNum==1){
    ...
    res=xSemaphoreGive(semaphore);
      if(res==pdTRUE){
        printf("key1 send semphr success,rest semphr num is %ld\n",uxQueueMessagesWaiting (semaphore));
      }else{
